...
**Also includes:** Shortcuts/Spotlight actions (Sync Now · Add Task) · Menu-bar quick-add with natural-language dates · Preview changes before applying (dry-run diff) · Undo last sync (restore vault files) · Multiple sync profiles (run several source→destination pipelines at once) · Auto-updater · Onboarding wizard · Global hotkey · GoodTask tag writeback · New task writeback to Obsidian inbox · Automatic file backups · Launch at login · macOS native (SwiftUI, no dependencies)
